...
- ✅ 2026-08-20: 아이폰 실기기 테스트 완료.
- ✅ 2026-08-20: 생일 축하(BIRTHDAY)·새 공지(NOTICE)도 푸시 대상에 추가 (`20260820160000_push_expand_birthday_notice.sql`). 이제 9종 중 댓글·좋아요만 제외, 나머지 전부 푸시로 나감.
- ✅ 2026-08-20: 조용한 시간대(밤 10시~오전 8시) 스위치 켬 (`PUSH_QUIET_HOURS_ENABLED=true`). 전체 공개 이전이지만, 이제 자동 알림도 나가므로 미리 켜기로 결정.

**알림 종류를 어떻게 구분하는지**: 새로운 로직을 추가한 게 아니라, `notifications` 테이블에 이미 저장되는 `type` 컬럼(MEAL/BULLETIN/ATTENDANCE/BIRTHDAY/NOTICE/MANUAL/COMMENT/LIKE)을 그대로 씁니다.
- **푸시를 보낼지 말지**: 트리거의 WHEN 절에 그 type이 들어있는지만 확인 (지금은 COMMENT·LIKE만 제외).
- **개인 알림 vs 전체 알림 구분이 따로 필요 없는 이유**: `notifications` 테이블은 이미 "받는 사람 1명당 1행"으로 저장됩니다. 생일 축하처럼 1명에게만 가는 알림은 행이 1개 생기고, 새 공지처럼 전 성도에게 가는 알림은 성도 수만큼 행이 여러 개 생깁니다. 트리거는 행 단위로 발동하므로, 각 수신자에게 각자의 기기로 알림이 갑니다 — 별도 "이건 개인용, 이건 전체용" 분기 로직이 필요 없습니다.
- **눌렀을 때 이동할 화면**: `send-push`의 `urlFor(type)`이 type별로 다른 탭으로 보냅니다 (생일→우리소식, 공지→홈, 나머지는 기존과 동일).
- ✅ 2026-08-20: 댓글(COMMENT)·좋아요(LIKE)는 건별 발송 대신 **하루 2번(오전 11시·오후 9시, 베트남 시각) 요약 푸시**로 전환 (`20260820170000_push_comment_like_digest.sql`).
