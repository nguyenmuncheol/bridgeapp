 ...
 *
 * 교회학교에 다니지 않는 자녀(미지정 · 출석 미적용)는 **주소록 교회학교 탭 · 출석체크 ·
 * 교회학교 인원수에서 모두 빠집니다.** 주소록은 교회학교 출석 관리용 명단이기 때문입니다.
 * 생일 달력·이달의 생일에는 그룹과 무관하게 나옵니다(생일이 입력되어 있기만 하면 됩니다).
 *
 * 그래서 두 값은 화면 동작이 같고, 구분은 뜻에만 있습니다:
 * - 미지정(빈 값): 부모가 아직 부서를 안 골랐음(입력 미완료)
 * - 출석 미적용: 교회학교에 다니지 않는 것이 확인됨
 */
